...
namespace SasquatchCAIRS.Controllers
{
    public class AuditLogManagementController : Controller
    {
        ...
        {
            // Create blank list of AuditLogs and fill with all AuditLogs for the given request ID
            List<SasquatchCAIRS.AuditLog> requestLogs = new List<AuditLog>();
            foreach (AuditLog log in (_db.AuditLogs.Where(r => r.UserID == userID && r.AuditDate.Date > startDate.Date && r.AuditDate.Date < endDate.Date))) {
                requestLogs.Add(log);
            }
